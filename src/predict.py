...
def visualize_predictions(csv_file):
    # 读取 CSV 文件
    df = pd.read_csv(csv_file, header=None)
    
    # 绘制折线图
    plt.figure(figsize=(10, 6))
    x = range(1, len(df) + 1)  # x 轴为数据点的索引
    for i in range(len(df.columns)):
        plt.plot(x, df[i], label=f'Condition {i+1}')  # 每一列作为一个折线
    plt.xlabel('Case')
...
def visualize_predictions(csv_file, condition_names):
    # 读取 CSV 文件
    df = pd.read_csv(csv_file, header=None)
    ...
    x = range(1, len(df) + 1)  # x 轴为数据点的索引
    for i in range(len(df.columns)):
        plt.plot(x, df[i], label=condition_names[i])  # 每一列作为一个折线，使用条件名称作为标签
    plt.xlabel('Case')
    plt.ylabel('Prediction Probability')
    plt.title('Predictions')
    plt.legend()
...
def main(valid_paths_csv, output_dir):
    ...
    # 调用函数来可视化预测
    condition_names = ['Abnormality', 'ACL Tear', 'Meniscal Tear']  # 设置条件名称
    visualize_predictions(output_file, condition_names)
